...
time = dt.now().strftime('%B %d, %Y at %I %p')
now = (day + ", " + time)
...
current_conditions = browser.find_element_by_css_selector('#dp0-details-narrative').text
...
server.sendmail(from_addr, to_addr, current)
server.sendmail(from_addr, to_addr, tomorrow)
server.sendmail(from_addr, to_addr, pollen_level)
server.sendmail(from_addr, to_addr, asthma_level)
